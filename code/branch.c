...
Branch_Validate(Bank *bank, BranchID branchID)
{
  ...
  for (int a = 0; a < branch->numberAccounts; a++) {
    sem_wait(&branch->accounts[a].accLock);
  }
  for (int a = 0; a < branch->numberAccounts; a++) {
  ...
  if (total != branch->balance) {
    ...
    for (int a = 0; a < branch->numberAccounts; a++) {
      sem_post(&branch->accounts[a].accLock);
    }
    return -1;
  }
...
Branch_Compare(Branch *branch1, Branch *branch2)
{
  ...
  for (int i = 0; i < branch1->numberAccounts; i++) {
    sem_wait(&branch1->accounts[i].accLock);
    sem_wait(&branch2->accounts[i].accLock);
  }
  ...
  for (int i = 0; i < branch1->numberAccounts; i++) {
    sem_post(&branch1->accounts[i].accLock);
    sem_post(&branch2->accounts[i].accLock);
  }
