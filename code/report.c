...
{
  sem_wait(&bank->reportLock);
  bank->workersLeft--;
  if(bank->workersLeft != 0){
    sem_post(&bank->reportLock);
    sem_wait(&bank->dayLock);
    return 0;
  }
  Report *rpt = bank->report;
  assert(rpt); Y;
  ...
  if (rpt->numReports >= MAX_NUM_REPORTS) {
    for(int i = 0; i < bank->numWorkers - bank->workersLeft; i++){
      sem_post(&bank->dayLock);
    }
    bank->workersLeft = bank->numWorkers;
    sem_post(&bank->reportLock);
      // We've run out of report storage for the bank
      return -1;
  }
